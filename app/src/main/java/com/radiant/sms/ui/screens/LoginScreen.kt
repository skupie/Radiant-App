...
fun LoginScreen(nav: NavController, vm: AuthViewModel = viewModel()) {
    ...
    LaunchedEffect(state.tokenPresent, state.role) {
        if (state.tokenPresent) {
            ...
            if (role == "admin") nav.navigate(Routes.ADMIN_HOME) { popUpTo(Routes.LOGIN) { inclusive = true } }
            else nav.navigate(Routes.MEMBER_HOME) { popUpTo(Routes.LOGIN) { inclusive = true } }
        }
    }

    Scaffold(topBar = { TopAppBar(title = { Text("RadiantSMS Login") }) }) { padding ->
        Column(
            modifier = Modifier
                .padding(padding)
                .padding(16.dp)
